...
public class SectCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sect")
                ...
                // NEW: Settings command for friendly fire
    ...
    private static int setFriendlyFire(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        sect.setFriendlyFire(enabled);
        manager.setDirty();
        String status = enabled ? "§aenabled" : "§cdisabled";
        context.getSource().sendSuccess(() -> Component.literal("§eFriendly fire has been " + status + "§e!"), true);
        broadcastToSect(sect, context.getSource().getServer(),
                "§e" + player.getScoreboardName() + " has " + (enabled ? "§aenabled" : "§cdisabled") + " §efriendly fire!");
        return 1;
    }
    ...
    private static int createSect(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ServerPlayer player = context.getSource().getPlayerOrException();
        String name = normalizeSectName(StringArgumentType.getString(context, "name"));
        if (!isValidSectName(name, context)) return 0;
        SectManager manager = getManager(context);
        if (manager == null) {
            context.getSource().sendFailure(Component.literal("§cSect system not available!"));
            return 0;
    ...
    private static int declineInvite(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ServerPlayer player = context.getSource().getPlayerOrException();
        String sectName = normalizeSectName(StringArgumentType.getString(context, "sectname"));
        SectManager manager = getManager(context);
        if (manager == null) {
            context.getSource().sendFailure(Component.literal("§cSect system not available!"));
            return 0;
    ...
    private static int promotePlayer(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        // MODIFIED: Bypass recommendations - only check merit for Elder promotion
        if (newRank == SectRank.ELDER) {
            ...
            // Recommendations check removed - manual promotion bypasses recommendation requirement
        }
    ...
    private static int acceptAlly(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ServerPlayer player = context.getSource().getPlayerOrException();
        String requestingSectName = normalizeSectName(StringArgumentType.getString(context, "sectname"));
        SectManager manager = getManager(context);
        if (manager == null) {
            context.getSource().sendFailure(Component.literal("§cSect system not available!"));
            return 0;
    ...
    private static int displaySectInfo(CommandContext<CommandSourceStack> context, ServerPlayer player, String target) throws CommandSyntaxException {
        ...
        Sect finalSect = sect;
        context.getSource().sendSuccess(() -> Component.literal("§6=== " + (target == null ? "Your Sect" : "Sect '" + finalSect.getName() + "'") + " Info ==="), false);
        Sect finalSect1 = sect;
        context.getSource().sendSuccess(() -> Component.literal("§eSect: §f" + finalSect1.getName()), false);
        ...
        context.getSource().sendSuccess(() -> Component.literal("§eMembers (" + finalSect3.getMembers().size() + "):"), false);
    ...
    private static int recommendPlayer(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        // Check promotion immediately when recommendations are added (auto-promotion still requires recommendations)
    ...
    private static int showMerit(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        if (member.getRank() == SectRank.OUTER) {
            ...
            int neededRecommendations = 3;
            context.getSource().sendSuccess(() -> Component.literal("§7Next Rank: §6Elder §7(§610000§7 merit + §6" + neededRecommendations + "§7 recommendations for auto-promotion)"), false);
            ...
            context.getSource().sendSuccess(() -> Component.literal("§7(§e/sect promote§7 can bypass recommendations with 10000 merit)"), false);
            ...
            context.getSource().sendSuccess(() -> Component.literal("§7Promotion to Sect Master must be done manually."), false);
    ...
    private static String getGradientColor(float progress, String startColor, String endColor) {
        return progress < 0.5f ? startColor : endColor;
    }
    ...
    private static int claimMissionSubmissions(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        Map<UUID, List<ItemStack>> submissions = sect.getSectBankItems();
        ...
        context.getSource().sendSuccess(() -> Component.literal("§aClaimed §e" + finalTotalItems + "§a items (§e" + finalTotalStacks + "§a stacks) from sect bank!"), false);
    ...
    private static int listSects(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        for (Map.Entry<String, Sect> entry : sortedSects) {
            ...
            MutableComponent hoverText = Component.literal("§6" + sectName + "\n")
                    ...
                    .append(Component.literal("§aFriendly Fire: " + (sect.isFriendlyFire() ? "§aEnabled" : "§cDisabled") + "\n"))
        ...
        if (context.getSource().getEntity() instanceof ServerPlayer) {
            ServerPlayer p = (ServerPlayer) context.getSource().getEntity();
            Sect playerSect = manager.getPlayerSect(p.getUUID());
            if (playerSect == null) {
    ...
    private static int renameSect(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ServerPlayer player = context.getSource().getPlayerOrException();
        String newName = normalizeSectName(StringArgumentType.getString(context, "new_name"));
        if (!isValidSectName(newName, context)) return 0;
        SectManager manager = getManager(context);
        if (manager == null) {
            context.getSource().sendFailure(Component.literal("§cSect system not available!"));
            return 0;
        ...
        for (String enemyName : enemies) {
            ...
            if (enemySect != null) {
                enemySect.removeEnemy(oldName);
                enemySect.addEnemy(newName);
            }
        }
    ...
    private static void copySectData(Sect source, Sect destination) {
        ...
        for (Map.Entry<UUID, Integer> entry : source.playerMerit.entrySet()) {
            destination.addPlayerMerit(entry.getKey(), entry.getValue());
        }
    ...
    private static CompletableFuture<Suggestions> suggestEnemySectNames(CommandContext<CommandSourceStack> context, SuggestionsBuilder builder) {
        ...
        if (player != null) {
            ...
            if (manager != null) {
                Sect sect = manager.getPlayerSect(player.getUUID());
                if (sect != null) {
                    for (String enemy : sect.getEnemies()) {
    ...
    private static int removeEnemy(CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        SectMember member = sect.getMember(player.getUUID());
        if (member == null || (member.getRank() != SectRank.ELDER && member.getRank() != SectRank.SECT_MASTER)) {
            context.getSource().sendFailure(Component.literal("§cOnly Elders and Sect Masters can remove enemies!"));
            return 0;
        }
        if (!sect.isEnemy(enemySectName)) {
            context.getSource().sendFailure(Component.literal("§c" + enemySectName + " is not your enemy!"));
            return 0;
        }
        sect.removeEnemy(enemySectName);
        manager.setDirty();
        context.getSource().sendSuccess(() -> Component.literal(
                "§aRemoved §e" + enemySectName + "§a from enemies list."
        ), true);
        broadcastToSect(sect, context.getSource().getServer(),
                "§a" + enemySectName + " is no longer an enemy.");
        return 1;
    }
    ...
    public static int help(CommandContext<CommandSourceStack> context) {
        ...
        sendHelpEntry(player, "/sect settings friendlyfire <true/false>",
                ...
                "Rank: §cElder§7+\n§6Note: Can bypass recommendations with 10000 merit");
        sendHelpEntry(player, "/sect enemy <sect>",
                ...
                "Rank: §cElder§7+");
        sendHelpEntry(player, "/sect enemy remove <sect>",
                "Remove a sect from enemies list",
                "Usage: /sect enemy remove <sectName>\nRestore neutral relations",
                "Rank: §cElder§7+");
        ...
        sendHelpEntry(player, "/sect promote <player>",
                ...
                "Rank: §6Sect Master\n§6Note: Bypasses recommendation requirement!");
