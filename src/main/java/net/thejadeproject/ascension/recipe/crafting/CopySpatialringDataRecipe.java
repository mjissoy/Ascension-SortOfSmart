...
public class CopySpatialringDataRecipe extends ShapedRecipe {
    public CopySpatialringDataRecipe(final String group, CraftingBookCategory category, ShapedRecipePattern pattern, final ItemStack recipeOutput) {
        super(group, category, pattern, recipeOutput);
    }

    public CopySpatialringDataRecipe(ShapedRecipe shapedRecipe) {
    ...
    public static class Serializer implements RecipeSerializer<CopySpatialringDataRecipe> {
        private static final MapCodec<CopySpatialringDataRecipe> CODEC = ShapedRecipe.Serializer.CODEC.xmap(CopySpatialringDataRecipe::new, $ -> $);
        private static final StreamCodec<RegistryFriendlyByteBuf, CopySpatialringDataRecipe> STREAM_CODEC = RecipeSerializer.SHAPED_RECIPE.streamCodec().map(CopySpatialringDataRecipe::new, CopySpatialringDataRecipe::new);
        @Override
        public @NotNull MapCodec<CopySpatialringDataRecipe> codec() {
            return CODEC;
        }
        public @NotNull StreamCodec<RegistryFriendlyByteBuf, CopySpatialringDataRecipe> streamCodec() {
            return STREAM_CODEC;
        }
    }
